...
def p_declaracion_con_tipo(p):
    ...
    p[0] = ("declaracion", p[1], p[2], p[4], p[6])

def p_declaracion_inferida(p):
    """declaracion : VAL ID ASIGNAR expresion
                   | VAR ID ASIGNAR expresion"""
    p[0] = ("declaracion", p[1], p[2], None, p[4])
...
def p_lectura(p):
    "lectura : READLN PAR_IZQ PAR_DER"
    p[0] = ("readln",)

def p_expresion_lectura(p):
    "expresion : lectura"
    p[0] = p[1]

def p_estructura_datos_array(p):
    "estructura_datos : ARRAYOF PAR_IZQ argumentos PAR_DER"
    p[0] = ("array", p[3])

def p_if_else(p):
    "estructura_if : IF PAR_IZQ expresion PAR_DER bloque ELSE bloque"
...
def p_estructura_datos_map(p):
    "estructura_datos : MAPOF PAR_IZQ pares PAR_DER"
    p[0] = ("map", p[3])
...
def p_par(p):
    "par : expresion TO expresion"
    p[0] = (p[1], p[3])

def p_while(p):
    "estructura_while : WHILE PAR_IZQ expresion PAR_DER bloque"
    p[0] = ("while", p[3], p[5])

def p_funcion_expresion(p):
    "declaracion_funcion : FUN ID PAR_IZQ parametros PAR_DER DOS_PUNTOS TIPO ASIGNAR expresion"
    p[0] = ("funcion-expr", p[2], p[4], p[7], p[9])
